...
updateGroundVoid(this.ground || this.scene.userData.ground, this.poolGroup);
  ...
  _scheduleDeferredStartupWork() {
    if (this._startupPerf.deferredInitStarted) return;
    this._startupPerf.deferredInitStarted = true;

    const perf = window.__POOL_PERF || {};
    const startHeavy = async () => {
      if (this._startupPerf.heavyFxReady) return;

      this._startupPerf.prepassEnabled = true;

      const dirLight = this.scene?.userData?.dirLight;
      const upgradedShadowMapSize = perf.upgradedShadowMapSize ?? 2048;
      if (dirLight && dirLight.shadow?.mapSize) {
        dirLight.shadow.mapSize.set(upgradedShadowMapSize, upgradedShadowMapSize);
      ...
      try {
        ...
        console.error("[PoolApp] Caustics init failed; continuing without caustics.", err);
        this.caustics = null;
      }
      ...
      const targetDpr = perf.interactionMaxPixelRatio ?? 2;
      this.renderer?.setPixelRatio?.(Math.min(window.devicePixelRatio || 1, targetDpr));
      this._startupPerf.heavyFxReady = true;
    };

    const kickoff = () => {
      const idle = window.requestIdleCallback || ((cb) => setTimeout(cb, perf.waterPrepassDelayMs ?? 1500));
      idle(() => { startHeavy().catch((err) => console.error("[PoolApp] Deferred startup failed", err)); });
    };

    this._afterFirstInteraction(kickoff);
    setTimeout(kickoff, perf.waterPrepassDelayMs ?? 1500);
  }
  ...
  async start() {
    ...
    await this.rebuildPoolForCurrentShape();
    this._scheduleDeferredStartupWork();
...
if (this._startupPerf.prepassEnabled && _poolWater && _poolU && this._waterInteriorRT) {
